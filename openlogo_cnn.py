...
# 卷积层1为64个滤波器，卷积核为3*3，默认填充为与原数组相同大小，激活函数选用relu
network = conv_2d(network, 64, 3, activation='relu')
# 最大池化层1
network = max_pool_2d(network, 2)
# 卷积层2为128个滤波器，卷积核为3*3，默认填充为与原数组相同大小，激活函数选用relu
network = conv_2d(network, 128, 3, activation='relu')
# 卷积层2为256个滤波器，卷积核为3*3，默认填充为与原数组相同大小，激活函数选用relu
network = conv_2d(network, 256, 3, activation='relu')
# 最大池化层2
network = max_pool_2d(network, 2)
# 全连接层1为512个结点，激活函数选用relu
network = fully_connected(network, 512 , activation='relu')
# 随机失活层，用于在训练过程中随机丢弃一些数据以防止过拟合，设定丢弃一半
network = dropout(network, 0.5)
# 全连接层2为10个结点，激活函数选用softmax，也即输出层
